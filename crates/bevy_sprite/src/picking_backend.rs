...
pub fn sprite_picking(
    ...
    let mut sorted_sprites: Vec<_> = sprite_query
        .iter()
        .filter(|x| !x.3.affine().is_nan())
        .collect();
    sorted_sprites.sort_by_key(|x| Reverse(FloatOrd(x.3.translation().z)));
    ...
    for (pointer, location) in pointers.iter().filter_map(|(pointer, pointer_location)| {
        ...
        let picks: Vec<(Entity, HitData)> = sorted_sprites
            ...
            .filter_map(|(entity, atlas, sprite, sprite_transform, pickable, ..)| {
                if blocked {
                    return None;
                }
                ...
                let extents = match (sprite.custom_size, atlas) {
                    ...
                    (None, None) => images.get(&sprite.texture)?.size().as_vec2(),
                    (None, Some(atlas)) => texture_atlas_layout
                        ...
                        .map_or(images.get(&sprite.texture)?.size().as_vec2(), |rect| {
                            rect.size().as_vec2()
                        }),
                };
