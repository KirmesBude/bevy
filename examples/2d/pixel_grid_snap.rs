...
fn setup_sprite(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.spawn((
        Sprite::from(asset_server.load("pixel/bevy_pixel_dark.png")),
    ...
    commands.spawn((
        Sprite::from(asset_server.load("pixel/bevy_pixel_light.png")),
        Transform::from_xyz(-40., -20., 2.),
        Rotate,
        HIGH_RES_LAYERS,
    ));
...
fn setup_camera(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    ...
    // spawn the canvas
    commands.spawn((Sprite::from(image_handle), Canvas, HIGH_RES_LAYERS));
